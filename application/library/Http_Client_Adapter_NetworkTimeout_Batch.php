...
class Http_Client_Adapter_NetworkTimeout_Batch implements Zend_Http_Client_Adapter_Interface, Zend_Http_Client_Adapter_Stream {
/**
    ...
    {
        $this->logger = new Logger(get_class(),"MBAT");
    }
...
//        while (($line = @fgets($this->socket)) !== false && (!$info['timed_out'])) {
        $start_time = time();
        $diff_arr = array();
        while(!feof($this->socket) && (!$info['timed_out'])){
            ...
            if(!$gotStatus){
                if($diff > $timeout){
                    ...
                    $this->close();
                    require_once 'Zend/Http/Client/Adapter/Exception.php';
                    throw new Zend_Http_Client_Adapter_Exception('Network Read Time out Exception');
                }
                usleep(2 * 100000);//polling per 0.2 second
                if(!$diff_arr[$diff]){
                    $this->logger->debug('polling['.$diff.']');
                    $diff_arr[$diff] = true;
                }
            }
        }
